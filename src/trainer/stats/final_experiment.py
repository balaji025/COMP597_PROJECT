...
class NvmlHelper:
    ...
    def sample(self) -> Dict[str, Optional[float]]:
        ...
        try:
            # millijoules, cumulative, available on many recent NVIDIA GPUs
...
class CodeCarbonWholeRun:
    """
    Use only for end-to-end energy mode.
    """
...
class FinalExperimentStats(base.TrainerStats):
    ...
    def start_train(self) -> None:
        ...
        if self.experiment_mode == "fine_grained":
        ...
        elif self.experiment_mode == "e2e_energy":
    ...
    def stop_train(self) -> None:
        ...
        summary = {
            ...
            "num_steps": len(self.step_timer.values_ms),
            "mean_step_ms": _mean(self.step_timer.values_ms),
            "std_step_ms": _std(self.step_timer.values_ms),
            "mean_forward_ms": _mean(self.forward_timer.values_ms),
            "std_forward_ms": _std(self.forward_timer.values_ms),
            "mean_backward_ms": _mean(self.backward_timer.values_ms),
            "std_backward_ms": _std(self.backward_timer.values_ms),
            "mean_optimizer_ms": _mean(self.optimizer_timer.values_ms),
            "std_optimizer_ms": _std(self.optimizer_timer.values_ms),
    ...
    def log_loss(self, loss: torch.Tensor) -> None:
        self.losses.append(float(loss.detach().float().item()))
    ...
    def log_step(self) -> None:
        # Intentionally minimal to avoid measurement overhead
        pass

    def log_stats(self) -> None:
        # Files are already written in stop_train()
        pass
